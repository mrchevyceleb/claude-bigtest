...
## Phase 9: FIX AND RETEST LOOP (MANDATORY)

**CRITICAL: You MUST loop until score reaches 100/100.**

After generating the report:

### 9.1 If Score < 100: FIX ALL ISSUES

1. **DO NOT STOP** - You are not done until score = 100
2. **Fix all issues** - Starting with CRITICAL, then MAJOR, then MINOR
3. **Use proper tools** - Edit files, fix bugs, improve code
4. **Track progress** - Use TodoWrite to track each fix

### 9.2 After Fixing: RETEST
...
### 9.3 Continue Loop Until Score = 100

```
WHILE score < 100:
    1. Fix all issues found in last test
    2. Retest entire application
    3. Generate new report
    4. Calculate new score

WHEN score = 100:
    Output: <promise>PRODUCTION_READY</promise>
```
...
Issues Fixed: [list]
New Score: [Y]/100
Status: [CONTINUING / PRODUCTION_READY]
```

### 9.5 Exit Conditions

- **SUCCESS**: Score = 100 → Output `<promise>PRODUCTION_READY</promise>`
- **BLOCKED**: Cannot fix an issue (needs user input, external dependency) → Output `<promise>TESTING_BLOCKED</promise>` with explanation

**NEVER stop at a score below 100 unless blocked!**
...
10. **LOOP UNTIL 100** - NEVER report a score and stop. Fix issues and retest until score = 100!
